...
return (
“<span style='background:" + bg + ";color:" + fc + ";border:1px solid " + fc + "44;"
...
for label, val, kind in all_levels:
    ...
    if kind == "iv":
        ...
        lines.append(
            ...
            "<span style='font-size:12px;" + lbl_s + "'>📘 " + label + " $" + format(val, ",.2f") + "</span>" + badge + "</div>"
        )
    else:
        if price <= val:
            ...
            lbl_s = "color:#78350f;"
            badge = "<span style='color:#b45309;font-size:11px;font-weight:600;'>⚠ " + str(pct) + "% away · watch</span>"
        else:
        ...
        lines.append(
            "<div style='margin:2px 0;" + row_s + "padding:3px 8px;border-radius:3px;"
            "display:flex;justify-content:space-between;align-items:center;gap:8px;'>"
            "<span style='font-size:12px;" + lbl_s + "'>" + label + " <strong>$" + format(val, ",.2f") + "</strong></span>" + badge + "</div>"
        )
...
for idx in range(header_row + 1, len(df)):
    ...
    queen = "Q" in queen_raw or "q❤" in queen_raw.lower() or "❤" in queen_raw
...
st.write(”**Columns detected:**”, col_map)
st.write(”**Rows parsed:**”, len(portfolio))
st.write(”**Data date found:**”, sheet_label_date)
st.dataframe(df.head(12), use_container_width=True)
if not portfolio:
st.warning(“No US stocks found. Share the debug table above so the column mapping can be fixed.”)
st.stop()
...
if earnings_map:
items = “  ·  “.join(
“<b>” + t + “</b> “ + e[“label”] + “ (” + str(e[“days_away”]) + “d)”
...
"padding:10px 16px;margin-bottom:4px;font-size:13px;'>”
“🔔 <b>Upcoming earnings:</b>  ” + items + “</div>”,
unsafe_allow_html=True
)
...
for col, lbl in zip(hcols, [“Ticker”,“Company”,“Live price”,“IV discount”,“Buy point ladder  (IV → S1 → S2 → S3…)”]):
col.markdown(
“<span style='font-size:11px;font-weight:700;color:#6b7280;"
"text-transform:uppercase;letter-spacing:0.06em;'>” + lbl + “</span>”,
unsafe_allow_html=True
)
...
with c2:
st.markdown(”<div style='padding-top:4px;font-size:13px;color:#374151;'>” + r[“name”] + “</div>”, unsafe_allow_html=True)
with c3:
ps = “$” + format(r[“price”], “,.2f”) if r[“price”] else “-”
st.markdown(”<div style='padding-top:4px;font-weight:700;font-size:14px;'>” + ps + “</div>”, unsafe_allow_html=True)
with c4:
